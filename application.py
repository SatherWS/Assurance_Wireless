...
application.secret_key = "aknfn348h23h5rwainfoanfw4"
mysql = pymysql.connect(host='localhost', user='root', port=3306, passwd='toor', db='awla_db')
...
@application.route('/new-ticket', methods=['POST', 'GET'])
def new_ticket():
...
@application.route('/submit-ticket')
def submit_ticket():
...
@application.route('/messaging/<ticket_id>', methods=['POST', 'GET'])
...
def check_address(addr):
    ...
    try:
        ...
        gps.append(location.latitude)
        gps.append(location.longitude)
        return gps
...
def add_application(app):
    ...
    language, zipcode, street, city, state, latitude, longitude) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
...
def register():
    ...
    if request.method == 'POST':
        ...
        address_str = app[6]+' '+app[8] +' '+app[5]
        gps_coords = check_address(address_str)
        
        if gps_coords != False:
            app.append(gps_coords[0])
            app.append(gps_coords[1])
...
def get_user_tickets(user_email):
    try:
        curs = mysql.cursor()
        sql = "select question from support_tickets where requester = %s"
...
def show_status():
    ...
    if session['email']:
        ...
        if tickets != False:
            return render_template("status.html", rs=rs, tickets=tickets)
        else:
...
def map():
    ...
    curs = mysql.cursor()
    sql = "select latitude, longitude from applications"
    curs.execute(sql)
    data = curs.fetchall()
    rs = data[0]
    return render_template("map.html", rs=rs)
...
@application.route("/search-apps", methods=['POST','GET'])
def search_apps():
...
@application.route("/process_apps", methods=['GET', 'POST'])
def process_apps():
...
@application.route('/comment-queue', methods=['POST', 'GET'])
def show_tickets():
    if session['admin']:
        curs = mysql.cursor()
        sql = """select id, status, category, question, requester, acceptor, time_created 
...
@application.route('/select-ticket/<ticket_id>', methods=['POST', 'GET'])
...
@application.route("/get-chat-log")
def get_chat_log():
    txt = get_data()
...
@application.route('/message-action', methods=['POST'])
def message_action():
